...
const UserFriendsActionsButton = ({ userId, buttonClassName }: UserFriendsActionsButtonProps) => {
  ...
  const hasFriendRequestSent = !!sentFriendRequests.find((request) => request.to === userId);
  ...
  if (hasFriendRequestSent) return <CancelSentFriendRequestButton userId={userId} />;

  if (hasReceiveFriendRequest) return <RespondToFriendRequestButton userId={userId} />;
